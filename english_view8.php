...
// 2. Query specifically for the Week 8 table
$result = mysqli_query($conn, "SELECT * FROM english_quiz8");
?>
...
<head>
    ...
    <title>Kidemy | English Week 8 Quiz</title>
    <style>
        /* Consistent Blue Theme matching your previous views */
...
<div class="quiz-card">
    <a href="English_lesson8.php" class="back-link">‹ Back to Lesson</a>
    
    <div class="header">
        <small>English 4 - Quarter 1</small>
        <h2>Week 8: Story Elements & Summarizing</h2> 
    </div>

    

    <form action="english_submit8.php" method="POST">
        <?php if($result && mysqli_num_rows($result) > 0): ?>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p style="text-align:center; color:#666;">Waiting for questions... Ensure the english_quiz8 table is ready.</p>
        <?php endif; ?>

        <button type="submit" class="submit-btn">Finish Quarter 1 Quiz</button>
    </form>
</div>
